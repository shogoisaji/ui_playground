...
class _ParticlePageState extends State<ParticlePage> with TickerProviderStateMixin {
  ...
  Offset _clipCenterPosition = Offset(0, 0);
  @override
  void initState() {
    super.initState();

    _startAnimationController = AnimationController(vsync: this, duration: const Duration(milliseconds: 3000));
    _startAnimation = CurvedAnimation(parent: _startAnimationController, curve: Curves.easeOutQuart);
    Future.delayed(const Duration(milliseconds: 500), () {
      _startAnimationController.forward();
    });

    _circleMoveAnimationController = AnimationController(vsync: this, duration: const Duration(milliseconds: 3000));
    _circleMoveAnimation = CurvedAnimation(parent: _circleMoveAnimationController, curve: Curves.easeInOut);
      ...
      _circleMoveAnimation.addListener(() {
        setState(() {
          ...
          _circleRadius = size.width > size.height
              ? (size.width + 100) / 2 * _circleMoveAnimation.value + _initialRadius
              : (size.height + 100) / 2 * _circleMoveAnimation.value + _initialRadius;
        });
      });
      _startAnimation.addListener(() {
        setState(() {
          _clipCenterPosition = Offset(
              MediaQuery.sizeOf(context).width / 2, (MediaQuery.sizeOf(context).height / 2 * _startAnimation.value));
        });
      });
    });
  }
    ...
    return Scaffold(
      ...
      body: Stack(
        ...
        children: [
          ...
          ClipPath(
            clipper: MyClipper(centerPosition: _clipCenterPosition, radius: _circleRadius),
            child: Container(
...
class ParticlePainter extends CustomPainter {
  ...
  ParticlePainter({required this.particles, required this.centerPosition, required this.circleRadius});

  @override
  void paint(Canvas canvas, Size size) {
    Offset calculatePosition(Particle particle) {
      return Offset(centerPosition.dx + cos(particle.angle) * circleRadius + particle.x,
          centerPosition.dy + sin(particle.angle) * circleRadius + particle.y);
    }

    for (var particle in particles) {
      var paint = Paint()
        ..color = Colors.grey[800]!.withOpacity((particle.opacity / 255).clamp(0, 1))
        ..style = PaintingStyle.fill;

      canvas.drawCircle(calculatePosition(particle), particle.particleRadius, paint);
    }
  }
